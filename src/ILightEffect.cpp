...
virtual void ILightEffect::addNode(std::shared_ptr<LightNode> node) {
	//TODO: Make sure that the node isn't already in the vector

	nodes->push_back(node);
}

virtual void ILightEffect::removeNode(std::shared_ptr<LightNode> node) {
...
virtual std::vector<std::shared_ptr<LightNode>>::iterator
	ILightEffect::begin() {
